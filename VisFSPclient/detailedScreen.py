...
# Form implementation generated from reading ui file 'detailedScreen.ui'
#
...
from PyQt5.QtWidgets import QApplication, QMainWindow, QTableWidgetItem, QStyle, QFormLayout, QWidget, QStyleFactory,QAbstractItemView,QHBoxLayout,QGridLayout,QMessageBox,QDialog
...
class Detailed(QDialog):
    ...
    def setupUi(self):
        ...
        self.statusLabel.setText('Статус: <font style="color: rgb(0, 255, 0);">good</font>')
        ...
        self.smallTable.setStyleSheet("background-color: white;"
                                "border-width: 0;"
                                "border-radius: 0;"
                                "border-color: white"
                                )
        ...
        self.bigTable.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.Stretch)
        self.bigTable.setHorizontalHeaderLabels(["Длина", "Ширина", "Положение","Диагональ","Поворот","Прямоуг-ть","Загрязн-е"])
        self.bigTable.setVerticalHeaderLabels(["Номинальные значения","Измеренные значения"])
        self.bigTable.setStyleSheet("background-color: white;"
                                "border-width: 0;"
                                "border-radius: 0;"
                                "border-color: white"
                                )
        ...
        self.previousButton.clicked.connect(lambda: self.new_detailed_win(int(self.Id)-1))   
        ...
        self.nextButton.clicked.connect(lambda: self.new_detailed_win(int(self.Id)+1))
    ...
    def setupImage(self,data):######################################################################################################## стереть data и убрать коментарий со следущей строки
        #data = json.dumps(data)
        data =  [[[5,5,7,10,10,5],[20,13,10,10,20,20]],[[6,8,10],[18,15,12]]]
    ...
    def setupTables(self):
        ...
        names2 = ['length_n', 'length_f', 'width_n', 'width_f', 'position_n', 'position_f', 'diag_n','diag_f', 'turn_n', 'turn_f', 'squareness_n', 'squareness_f',"spot_n","spot_f"]
        try:###################
            data = sql_test_module.sql_search_data(self.Id)
            self.setupImage(data["image"])

            stat = {'1':'rgb(0,150,0);">Норма</font>','2':'rgb(210,210,0);">Предупреждение</font>','3':'rgb(255,0,0);">Брак</font>'}
            data2 = sql_test_module.settingsData(data["tolerance"])[1]
            names3 = ['length_overlarge_' if float(data['length_f']) >= 0 else "length_extrasmall_",'width_overlarge_' if float(data['width_f']) > 0 else "width_extrasmall_","position_","diag_","turn_","squareness_","spot_"]

            self.statusLabel.setText('Статус: <font style="color:'+stat[str(data["status"])])


            for i in range(3):
                self.smallTable.setItem(0, i, QTableWidgetItem(str(data[names[i]])))



            for i in range(0,14,2):
                if i not in [10,8,12]:
                    self.bigTable.setItem(0, i//2, QTableWidgetItem(str(data[names2[i]])))
                    self.bigTable.setItem(1, i//2, QTableWidgetItem(str(data[names2[i+1]])))
                    if abs(float(data[names2[i+1]])) >= float(data2[names3[i//2]+"b"]):
                        self.bigTable.item(1,i//2).setForeground(QColor(255, 0, 0))
                    elif abs(float(data[names2[i+1]])) < float(data2[names3[i//2]+"w"]):
                        self.bigTable.item(1,i//2).setForeground(QColor(0, 150, 0))
                    else:
                        self.bigTable.item(1,i//2).setForeground(QColor(210, 210, 0))
                elif i == 10:
                    ...
                    if sq == "0":
                        ...
                        self.bigTable.item(1,5).setForeground(QColor(0, 150, 0))
                elif i == 8:
                    sq = abs(float(data["turn_f"]))
                    self.bigTable.setItem(0, 4, QTableWidgetItem("0"))
                    self.bigTable.setItem(1, 4, QTableWidgetItem(str(data["turn_f"])))
                    ...
                    self.bigTable.setItem(1, 6, QTableWidgetItem(str(data["spot_f"])))
                    if sq >= float(data2[names3[i//2]+"b"]):
                        self.bigTable.item(1,i//2).setForeground(QColor(255, 0, 0))
                    elif sq < float(data2[names3[i//2]+"w"]):
                        self.bigTable.item(1,i//2).setForeground(QColor(0, 150, 0))
