...
from PyQt5.QtWidgets import QApplication, QMainWindow, QTableWidgetItem, QStyle, QFormLayout, QWidget, QStyleFactory,QDateEdit, QLabel, QDialog
...
class Tableclass(QtWidgets.QWidget):
	def __init__(self,mainwindow):
		...
		self.Table.setHorizontalHeaderLabels(["Id", "Дата","Дефект","Статус"])
		...
		for i in range(len(data)):
			for j in range(4):
				if j == 3:
					...
					self.Table.item(i, 3).setBackground(colors[str(data[i]["status"])])
				elif j == 2:
					...
					self.Table.setItem(i, j, QTableWidgetItem(str(data[i][self.namesql[j]])))
...
class Finder(QDialog):
	...
	def __init__(self):
		...
		self.findbtn.setStyleSheet("background-color: white;"
		                                "border-width: 1;"
		                                "border-radius: 4;"
		                                "border-style: solid;"
		                                "border-color: rgb(0, 0, 0)"
	...
	def find(self):
		l = [self.toggle1.isChecked(),self.toggle2.isChecked(),self.toggle3.isChecked()]
