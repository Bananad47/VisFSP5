...
class Settings(QDialog):
    ...
    def setupUi(self):
        ...
        self.applybtn.setGeometry(QtCore.QRect(10, 590, 110, 50))
        self.applybtn.setObjectName("applybtn")
        self.applybtn.setText("Применить")
        self.applybtn.setStyleSheet("background-color: white;"
                                "border-width: 1;"
                                "border-radius: 4;"
                                "border-style: solid;"
                                "border-color: rgb(0, 0, 0)"
        ...
        self.applybtn.setFont(QtGui.QFont("Times", 12))
        iconap = QtGui.QIcon("icons/galochka.png")
        self.applybtn.setIcon(iconap)
        self.applybtn.setIconSize(QtCore.QSize(20, 20))
        self.applybtn.clicked.connect(self.updateData)
        ...
        self.password = PasswordClass(self)
...
#cd C:\Users\user\Desktop\VisFSPnew\VisFSP\
    ...
    def checkPassword(self):
        try:
            password = sql_test_module.getPassword()["password"]
            return True if password == self.password.text() else False
        except:
            self.errorsig.emit()
    ...
    def updateData(self):
        ...
        if not self.checkPassword():
                msg = QMessageBox()
                msg.setStyleSheet("border-width: 0;"
                            "border-radius: 0;"
                            "border-style: solid;"
                            "border-color: rgb(0, 0, 0)}")
        ...
        for i in tab2names:
            if not i.text().replace(".", "", 1).isdigit():
                ...
                msg.setStyleSheet("border-width: 0;"
                            ...
                            "border-style: solid;"
                            "border-color: rgb(0, 0, 0)}")
                msg.about(self,"Ошибка", "Введенные данные не являются числами")
                msg.setIcon(QMessageBox.Warning)
                return 0
